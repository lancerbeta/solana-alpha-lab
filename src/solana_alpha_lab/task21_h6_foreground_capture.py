"""Foreground-only H6 quote capture for the exact TASK-21 H0 population."""
...
HORIZON_ID = "H6"
NEXT_HORIZON_ID = "H24"
NEXT_ATOM_ID = "T21-A6S_H24_FOREGROUND_CAPTURE_V1"
NEXT_OFFSET_SECONDS = 86_400
OUTPUT_RELATIVE_ROOT = "local/task21_forward/h6_capture"
...
class Task21H6Error(RuntimeError):
    """H6 cannot safely proceed."""


class Task21H6AuthorityRequired(Task21H6Error):
    """The exact provider authority is absent."""


@dataclass(frozen=True, slots=True)
class Task21H6ExecutionGate:
    authority_phrase: str

    def __post_init__(self) -> None:
        if self.authority_phrase != ATOM_ID:
            raise Task21H6AuthorityRequired(
                "task21_h6_external_authority_phrase_mismatch"
            )
...
def _load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise Task21H6Error("json_root_invalid")
    return value
...
def _load_yaml(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise Task21H6Error("config_root_invalid")
    return value


def _utc(name: str, value: object) -> datetime:
    if not isinstance(value, str):
        raise Task21H6Error(f"{name}_must_be_text")
    try:
        parsed = datetime.fromisoformat(value.replace("Z", "+00:00"))
    except ValueError as exc:
        raise Task21H6Error(f"{name}_invalid") from exc
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise Task21H6Error(f"{name}_must_be_timezone_aware")
...
def _write_new(path: Path, payload: bytes) -> None:
    ...
    try:
        ...
        raise Task21H6Error("h6_create_only_collision") from exc
...
def validate_config(config: Mapping[str, Any], repo_root: Path) -> None:
    ...
    if (
        gate.get("gate_id") != "TASK21-H6-2026-07-31T13-50-34Z"
        or gate.get("earliest_at") != "2026-07-31T13:50:34.414367Z"
        or gate.get("latest_at") != "2026-07-31T14:00:34.414367Z"
        or gate.get("after_latest")
        != "WRITE_EXPLICIT_GAP_NO_PROVIDER_NO_BACKFILL"
    ):
        raise Task21H6Error("h6_time_gate_drift")
    h6 = config.get("h6", {})
    if (
        h6.get("panels_max") != 3
        or h6.get("provider_calls_per_panel_max") != CALLS_PER_PANEL_MAX
        or h6.get("provider_calls_total_max") != CALLS_TOTAL_MAX
        or h6.get("durable_local_bytes_max") != DURABLE_BYTES_MAX
        or h6.get("notionals_usd") != [10, 25, 50, 100]
    ):
        raise Task21H6Error("h6_cap_drift")
    ...
    if (
        ...
        raise Task21H6Error("h6_authority_boundary_drift")
...
def _load_members(config: Mapping[str, Any], repo_root: Path) -> list[dict[str, Any]]:
    ...
    for line in (repo_root / admission_item["path"]).read_text(
        ...
        if not isinstance(value, dict):
            raise Task21H6Error("h0_admission_event_invalid")
        members.append(value)
    if tuple(item.get("member_id") for item in members) != EXPECTED_MEMBER_IDS:
        raise Task21H6Error("h6_population_or_order_drift")
    if any(
        ...
        raise Task21H6Error("h6_member_contract_drift")
    return members
...
def _capture_member(
    *,
    ...
    if transport.attempts > CALLS_PER_PANEL_MAX:
        raise Task21H6Error("h6_panel_call_cap_exceeded")
    if not projections:
        raise Task21H6Error("h6_panel_has_no_evidence")

    member_id = member["member_id"]
    window_id = f"{member_id}-H6"
    window_root = run_root / f"member={member_id}" / "horizon=H6"
    envelopes = [
...
def _gap_receipt(
    *,
    ...
    run_id = "h6-gap-" + sha256_bytes(canonical_json_bytes(claim))[:16]
    run_root = output_root / f"run={run_id}"
    if run_root.exists():
        raise Task21H6Error("h6_gap_output_already_exists")
    backup_at = _utc(
    ...
    receipt = {
        "schema": "smial.task21.h6-runtime-receipt",
        "schema_version": SCHEMA_VERSION,
        "task_id": TASK_ID,
        "atom_id": ATOM_ID,
        "run_id": run_id,
        "status": "GAP",
        "reason": "H6_WINDOW_MISSED_NO_BACKFILL",
        "observed_blockers": {
            "recovery_backup_fresh_at_window_open": backup_age_at_earliest <= 24,
        ...
        "next_boundary": {
            ...
            "horizon_id": NEXT_HORIZON_ID,
            "status": "ACTIVE_WAITING_NOT_AUTHORIZED",
            "earliest_at": _utc_text(next_earliest),
            "latest_at": _utc_text(next_earliest + timedelta(minutes=10)),
...
def run_h6_foreground_capture(
    *,
    gate: Task21H6ExecutionGate | None,
    repo_root: Path,
    config_path: Path,
    ...
    if observed_at.tzinfo is None or observed_at.utcoffset() is None:
        raise Task21H6Error("runtime_now_must_be_timezone_aware")
    ...
    if observed_at < earliest:
        raise Task21H6Error("h6_window_not_open")
    output_root = (
    ...
    next_earliest = latest_h0 + timedelta(seconds=NEXT_OFFSET_SECONDS)
    if observed_at > latest:
        ...
        return _gap_receipt(
            ...
            recovery=recovery,
            provider_authority_present=isinstance(gate, Task21H6ExecutionGate),
        )

    if not isinstance(gate, Task21H6ExecutionGate):
        raise Task21H6AuthorityRequired(
            "task21_h6_external_execution_gate_required"
        )
    if output_root_override is not None and transport_factory is None:
        raise Task21H6Error("output_override_requires_injected_transport")

    recovery = _load_json(recovery_receipt_path)
    try:
        validate_recovery_freshness(recovery, now=observed_at)
    except Task21LiveShakedownError as exc:
        raise Task21H6Error(str(exc)) from exc
    free_bytes = (
    ...
    if free_bytes - DURABLE_BYTES_MAX < MIN_FREE_SPACE_AFTER_WRITE:
        raise Task21H6Error("h6_disk_pressure")
    ...
    run_root = output_root / f"run={run_id}"
    if run_root.exists():
        raise Task21H6Error("h6_run_output_already_exists")
    ...
    if calls > CALLS_TOTAL_MAX:
        raise Task21H6Error("h6_total_call_cap_exceeded")
    if received > RECEIVED_BYTES_MAX:
        raise Task21H6Error("h6_total_received_byte_cap_exceeded")
    ...
    receipt = {
        ...
        "next_boundary": {
            ...
            "status": (
                ...
                if complete
                else "H6_STOPPED_REVIEW_REQUIRED"
            ),
            "earliest_at": _utc_text(next_earliest),
            "latest_at": _utc_text(next_earliest + timedelta(minutes=10)),
            "provider_api_rpc_wss_calls_authorized": False,
    ...
    if stored > DURABLE_BYTES_MAX:
        raise Task21H6Error("h6_total_durable_byte_cap_exceeded")
    receipt["local_evidence"] = {
        "root": (
