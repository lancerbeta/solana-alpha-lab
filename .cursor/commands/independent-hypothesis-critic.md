...
If the pasted packet is `packet_version=1.1` or `1.2` and has no `session_id`: do
not return a critic result. Output `STATUS=INCOMPLETE_CRITIC_INPUT_PACKET` and
`OWNER NEXT=RE_RUN_FREEZE_AND_PASTE_PACKET_WITH_SESSION_ID`.

If the pasted packet is `packet_version=1.2` and has no `prior_memory`: do not
...
Historical `1.0` / `1.1` packets remain readable without `prior_memory`.

Packet `prior_memory` is the sole research-memory input. Do not open
ResearchStore or active RDP for prior recall.
