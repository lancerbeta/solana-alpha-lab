...
objective: >-
  ...
  HFIC-V1.2 Critic packet (critic packet_version=1.3) so an isolated Critic
  cannot miss an eligible HYPOTHESIS_VERSION solely because Forge lexical top-N
  omitted it. Keep historical critic 1.0/1.1/1.2 readable; 1.2 does not require
  prior_memory. Fail closed on capacity. Do not change rank_prior_candidate_ids
  or Prompt A search.
...
stop_conditions:
  ...
  - HISTORICAL_V12_PACKET_SEMANTICS_MUTATED
  - ACTIVE_RDP_WRITE
...
`CRITIC_INPUT_PACKET` as critic `packet_version=1.3` (`generator_prompt_version`
stays `HFIC-V1.2`). Isolated Critic uses that packet as sole research-memory
input. Historical critic `1.0` / `1.1` / `1.2` remain readable; `1.2` does not
require `prior_memory`. Capacity overflow is `PRIOR_MEMORY_CONTEXT_CAPACITY_EXCEEDED`
before persist. Ranker shortlist unchanged.
