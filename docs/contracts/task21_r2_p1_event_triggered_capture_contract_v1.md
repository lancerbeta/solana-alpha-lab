---
contract_id: CONTRACT-T21-R2-P1-EVENT-TRIGGERED-CAPTURE-001
...
# TASK-21 R2 P1 event-triggered foreground capture

## Outcome

Capture the second executable quote panel for the exact three outcome-blind R2
...
Each member becomes eligible only after its own accepted P0 completion plus
1,801 seconds. All three must be eligible before the first P1 provider call.
There is no narrow expiry window, but every P1 must remain inside the member's
24-hour total-span deadline measured from admission. A missed deadline is an
explicit gap, never a backfill or silent reschedule.

The population, order, mints, decimals, nomination identities, R2 acceptance,
admission event bytes, and all three P0 receipts are hash-bound. Quote, route,
...
provider failure retains evidence and stops without retrying or substituting a
member.
...
Neither P1 nor P2 alone authorizes TASK-22, A7, dataset promotion, or an alpha
claim.
