#!/usr/bin/env python3
"""Run one foreground organic-pressure H900 audition after owner authorization."""
...
RAW_ROOT = ROOT / "local/ordinary_recent_organic_pressure_h900_audition"
RUNTIME_RECEIPT_PATH = (
    ROOT
    / "docs/evidence/ordinary_recent_organic_pressure_h900_audition"
    / "a1_ordinary_recent_organic_pressure_h900_audition_runtime_receipt_v1.json"
)
...
def capture_envelope(*, observation_id: str, observed_at: str, body_sha256: str) -> dict[str, str]:
    payload = {
        ...
        "schema": "smial.ordinary-recent-organic-pressure-h900-audition.capture-envelope",
        "schema_version": "1.0",
    }
    return {
        **payload,
...
def attempt_reservation_document(*, started_at: str, policy_sha256: str) -> dict[str, object]:
    payload = {
        ...
        "provider_requests": 0,
        "schema": "smial.ordinary-recent-organic-pressure-h900-audition.attempt-reservation",
        "schema_version": "1.0",
...
def _load_policy() -> dict[str, object]:
    ...
    validate_policy(loaded, root=ROOT)
    return loaded
...
def _load_excluded_mints(path: Path) -> set[str]:
    try:
        raw = path.read_bytes()
    except OSError as exc:
        raise OrganicPressureError("PRIOR_MINT_EXCLUSION_INPUT_INVALID") from exc
    return _parse_excluded_mints(raw)
    ...
    *,
    ...
    try:
        ...
        receipt = {
            ...
            "provider_requests": exc.provider_requests or 0,
            "retries": 0,
            "fallbacks": 0,
            "execute_calls": 0,
            "observations": [],
    ...
    receipt["receipt_id"] = "EVIDENCE-ORDINARY-RECENT-ORGANIC-PRESSURE-H900-AUDITION-001"
