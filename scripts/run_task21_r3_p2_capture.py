#!/usr/bin/env python3
"""Run the authorized TASK-21 R3 P2 foreground capture."""
...
def main() -> int:
    ...
    try:
        receipt = run_event_triggered_p2_capture(
            ...
            repo_root=ROOT,
            config_path=ROOT / "configs/task21_r3_p2_event_triggered_capture_v1.yaml",
        )
    except Exception as exc:
        print("TASK21_R3_P2_CAPTURE: FAIL")
        print(f"ERROR_TYPE: {type(exc).__name__}")
        print(f"ERROR_CODE: {_safe_code(exc)}")
        return 1
    print("TASK21_R3_P2_CAPTURE: " + str(receipt["status"]))
