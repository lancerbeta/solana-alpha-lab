#!/usr/bin/env python3
"""Execute the TASK-37 RC002 H11 migration-clock capture."""
...
RUNTIME_RECEIPT_PATH = (
    ROOT / "docs/evidence/task37/a1_h11_migration_clock_capture_runtime_receipt_v1.json"
)
ACCEPTANCE_PATH = (
    ROOT / "docs/evidence/task37/a1_h11_migration_clock_capture_acceptance_v1.json"
)
READOUT_PATH = (
    ROOT / "docs/reports/task37/a1_h11_migration_clock_capture_owner_readout_v1.md"
)
...
def main() -> int:
    ...
    result = execute_capture(repo_root=ROOT, policy=policy)
    runtime = {
        "schema": "smial.task37.rc002-h11-migration-clock-capture.runtime",
        "schema_version": "1.0",
        "atom_id": ATOM_ID,
    ...
    acceptance = {
        "schema": "smial.task37.rc002-h11-migration-clock-capture.acceptance",
        "schema_version": "1.0",
        "as_of": measured.date().isoformat(),
        "atom_id": ATOM_ID,
        "receipt_id": "EVIDENCE-T37-RC002-H11-CLOCK-CAPTURE-001",
        "terminal_decision": result["terminal_decision"],
        "research_cycle_id": result["research_cycle_id"],
        "clock_sha256": result["clock_sha256"],
    ...
    sys.stdout.write(
        canonical_json({"terminal_decision": result["terminal_decision"]}).decode("utf-8")
        + "\n"
    )
    return 0
