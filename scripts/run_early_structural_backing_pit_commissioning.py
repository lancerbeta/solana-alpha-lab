#!/usr/bin/env python3
"""Run one foreground EARLY structural-backing PIT window after owner authorization."""
...
RAW_ROOT = ROOT / "local/early_structural_backing_pit_commissioning"
EVIDENCE_ROOT = ROOT / "docs/evidence/early_structural_backing_pit_commissioning"
...
def capture_envelope(*, observation_id: str, observed_at: str, body_sha256: str) -> dict[str, str]:
    payload = {
        ...
        "schema": "smial.early-structural-backing-pit-commissioning.capture-envelope",
        "schema_version": "1.0",
    }
    return {
        **payload,
...
def attempt_reservation_document(*, started_at: str, policy_sha256: str, window: str) -> dict[str, object]:
    payload = {
        "atom_id": ATOM_ID,
        "credential_reads": 0,
        "policy_sha256": policy_sha256,
        "provider_requests": 0,
        "schema": "smial.early-structural-backing-pit-commissioning.attempt-reservation",
        "schema_version": "1.0",
...
def _load_policy() -> dict[str, object]:
    ...
    validate_structural_backing_policy(loaded, root=ROOT)
    return loaded
...
def _receipt_path(window: str) -> Path:
    label = window.lower()
    return EVIDENCE_ROOT / f"a1_window_{label}_runtime_receipt_v1.json"
...
def run_capture(
    *,
    ...
    if window not in {"A", "B"}:
        raise OrganicPressureError("WINDOW_INVALID")
    selected_policy = dict(policy or _load_policy())
    validate_structural_backing_policy(selected_policy, root=ROOT)
    ...
    window_raw_root = raw_root / f"window_{window.lower()}"
    window_raw_root.mkdir(parents=True, exist_ok=True)
    reservation_path = window_raw_root / "campaign_reservation.json"
    _write_create_only(
        reservation_path,
        canonical_json({key: value for key, value in reservation.items() if key != "reservation_sha256"}),
    )
    raw_directory = window_raw_root / f"run={_format_utc(started_at).replace('-', '').replace(':', '')}"
    ...
    receipt["receipt_id"] = f"EVIDENCE-EARLY-STRUCTURAL-BACKING-PIT-WINDOW-{window}-001"
    ...
    if isinstance(score, Mapping):
        receipt["family_hint"] = decide_family({"terminal": score.get("terminal"), "score": score})
    ...
    target = receipt_path or _receipt_path(window)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--window", required=True, choices=("A", "B"))
