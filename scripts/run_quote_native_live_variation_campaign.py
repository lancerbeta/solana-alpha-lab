#!/usr/bin/env python3
"""Execute the authorized live outcome-blind quote-native variation campaign."""
...
RAW_ROOT = ROOT / "local/quote_native_live_variation_campaign"
...
def run_capture(*, authority_phrase: str, wave: str) -> dict[str, object]:
    ...
    receipt["receipt_id"] = "EVIDENCE-QUOTE-NATIVE-LIVE-VARIATION-CAMPAIGN-001"
    _write_receipt(RUNTIME_RECEIPT_PATH, receipt, create_only=wave == "discovery")
    return receipt
...
def main() -> int:
    ...
    parser.add_argument("--wave", choices=("discovery", "t0", "due"), default="discovery")
    ...
    try:
        ...
        return 2
    campaign = receipt.get("campaign") if isinstance(receipt.get("campaign"), dict) else {}
    print(
        json.dumps(
            {
                "ok": True,
                ...
                "frozen_cell_count": len(receipt.get("frozen_cells") or []),
                "campaign_verdict": campaign.get("campaign_verdict"),
            },
            indent=2,
            sort_keys=True,
        )
