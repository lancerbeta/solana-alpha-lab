#!/usr/bin/env python3
"""Execute the authorized t0-friction → +15m quoted-liquidation look."""
...
from solana_alpha_lab.quote_native_evidence_fit_panel import PanelError  # noqa: E402
...
RAW_ROOT = ROOT / "local/quote_native_friction_h900_falsifier"
RUNTIME_RECEIPT_PATH = (
    ROOT
    / "docs/evidence/quote_native_friction_h900_falsifier"
    / "a1_quote_native_friction_h900_falsifier_runtime_receipt_v1.json"
)
CONSUMED_RECEIPT_PATH = ROOT / CONSUMED_RECEIPT
...
def run_capture(*, authority_phrase: str, wave: str) -> dict[str, object]:
    ...
    _require(RUNTIME_RECEIPT_PATH.resolve() != CONSUMED_RECEIPT_PATH.resolve(), "CONSUMED_RECEIPT_PATH_COLLISION")
    if CONSUMED_RECEIPT_PATH.is_file():
        consumed = json.loads(CONSUMED_RECEIPT_PATH.read_text(encoding="utf-8"))
        _require(isinstance(consumed, dict), "CONSUMED_RECEIPT_INVALID")
        _require(consumed.get("atom_id") == CONSUMED_ATOM, "CONSUMED_RECEIPT_ATOM_DRIFT")
    policy = _load_policy()
    ...
    if wave == "due":
        ...
        _require(loaded.get("atom_id") == ATOM_ID, "OLD_DUE_AT_REBUILT_OR_OLD_RECEIPT_MUTATED")
        prior = loaded
    receipt = run_wave(
        policy,
        root=ROOT,
    ...
    receipt["receipt_id"] = "EVIDENCE-QUOTE-NATIVE-FRICTION-H900-FALSIFIER-001"
        ...
        json.dumps(
            {
                ...
                "panel_started_at": receipt.get("panel_started_at"),
                "quoted_buy_ids": receipt.get("quoted_buy_ids"),
                "mechanism_verdict": (receipt.get("mechanism") or {}).get("verdict")
                if isinstance(receipt.get("mechanism"), dict)
                else None,
            },
            indent=2,
            sort_keys=True,
