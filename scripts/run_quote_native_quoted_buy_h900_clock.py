#!/usr/bin/env python3
"""Execute the authorized new-clock quote-native +15m panel."""
...
def run_capture(*, authority_phrase: str, wave: str) -> dict[str, object]:
    ...
    _require(RUNTIME_RECEIPT_PATH.resolve() != OLD_RECEIPT_PATH.resolve(), "OLD_RECEIPT_PATH_COLLISION")
    if OLD_RECEIPT_PATH.is_file():
        old = json.loads(OLD_RECEIPT_PATH.read_text(encoding="utf-8"))
        _require(isinstance(old, dict), "OLD_RECEIPT_INVALID")
        _require(old.get("atom_id") == OLD_PANEL_ATOM, "OLD_RECEIPT_ATOM_DRIFT")
    policy = _load_policy()
    ...
    receipt["receipt_id"] = "EVIDENCE-QUOTE-NATIVE-QUOTED-BUY-H900-CLOCK-001"
