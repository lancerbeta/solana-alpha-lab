#!/usr/bin/env python3
"""Run the separately authorized TASK-21 R2 P1 foreground capture."""
...
def main() -> int:
    ...
    try:
        receipt = run_event_triggered_followup_capture(
            ...
            repo_root=ROOT,
            config_path=(
                ROOT / "configs/task21_r2_p1_event_triggered_capture_v1.yaml"
            ),
        )
    except Exception as exc:
        print("TASK21_R2_P1_CAPTURE: FAIL")
        print(f"ERROR_TYPE: {type(exc).__name__}")
        print(f"ERROR_CODE: {_safe_code(exc)}")
        return 1
    print("TASK21_R2_P1_CAPTURE: " + str(receipt["status"]))
    print(
        json.dumps(
            receipt,
            ensure_ascii=False,
