...
SCHEMA_PATH = ROOT / "catalog/schemas/task40_rc002_h11_bonding_curve_pda_gta_clock_capture.schema.json"
IDL_PATH = ROOT / "tests/fixtures/task08/pump_event_idl_subset_v1.json"
PDA_PATH = ROOT / "tests/fixtures/task08/pump_bonding_curve_pda_subset_v1.json"
...
class Task40BondingCurvePdaGtaTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.policy = load_policy(CONFIG_PATH)
        cls.plan = load_pinned_pump_event_plan(IDL_PATH)
        cls.schema = json.loads(SCHEMA_PATH.read_text(encoding="utf-8"))
        cls.pda = load_pda_subset(PDA_PATH)

    def test_policy_matches_schema_owner_phrase_and_frozen_pda(self) -> None:
        ...
        address, bump = derive_bonding_curve_pda(self.pda)
        self.assertEqual(address, BONDING_CURVE)
        self.assertEqual(bump, 255)
        self.assertNotEqual(BONDING_CURVE, PUMP_PROGRAM_ID)
        self.assertNotEqual(BONDING_CURVE, POOL_ADDRESS)
        self.assertNotEqual(BONDING_CURVE, NAMED_MINT)

    def test_payload_is_a22_shape_with_curve_and_without_day_window(self) -> None:
        a22_policy = yaml.safe_load(A22_CONFIG.read_text(encoding="utf-8"))
        a22 = build_json_rpc_payload(a22_policy)
        curve = build_curve_gta_payload(self.policy, page_number=0)
        self.assertEqual(curve["method"], a22["method"])
        self.assertEqual(curve["params"][0], BONDING_CURVE)
        self.assertNotEqual(curve["params"][0], EXPECTED_POOL)
        self.assertNotEqual(curve["params"][0], PUMP_PROGRAM_ID)
        self.assertNotEqual(curve["params"][0], NAMED_MINT)
        a22_options = a22["params"][1]
        curve_options = curve["params"][1]
        self.assertEqual(curve_options["transactionDetails"], a22_options["transactionDetails"])
        self.assertEqual(curve_options["sortOrder"], "asc")
        self.assertEqual(curve_options["limit"], 1000)
        self.assertNotIn("blockTime", curve_options["filters"])
        self.assertIn("blockTime", a22_options["filters"])
        self.assertNotIn("paginationToken", curve_options)
    ...
    def test_empty_history_is_wrong_address(self) -> None:
        ...
        self.assertEqual(result["derived_pda"]["address"], BONDING_CURVE)

    def test_create_and_migration_below_h11_minima(self) -> None:
        pages = [
            _tx_row(
                logs=_pump_logs(self.plan, "CreateEvent"),
                keys=[PUMP_PROGRAM_ID, BONDING_CURVE, NAMED_MINT],
            ...
            _tx_row(
                ...
                keys=[PUMP_PROGRAM_ID, BONDING_CURVE, NAMED_MINT, POOL_ADDRESS],
