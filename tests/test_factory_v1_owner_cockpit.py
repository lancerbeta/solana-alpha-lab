...
class FactoryV1OwnerCockpitLiteTests(unittest.TestCase):
    def test_config_is_cockpit_lite_without_ui_package_or_vps(self) -> None:
        ...
        self.assertEqual(config["visible_nav"], ["HOME", "RESEARCH", "MARKET", "OPERATIONS", "ECONOMICS", "SYSTEM"])
        self.assertEqual(config["hidden_nav"], [])
    ...
    def test_packet_and_attention_are_visible_without_git_archaeology(self) -> None:
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmp:
            ...
            try:
                ...
                self.assertNotIn("DEGRADED_PROCESS_ALIVE_BACKUP_UNKNOWN", home)
                self.assertIn('href="/market"', home)
                self.assertIn(">Рынок<", home)
                ...
                self.assertEqual(ops_model["operations"]["source_status"], "NOT_PRESENT")
                ...
                self.assertIn('href="/market"', operations)
                self.assertIn('href="/market"', economics)
                self.assertNotEqual((model.get("runtime") or {}).get("verdict"), "HEALTHY")
                self.assertNotEqual(model["system_operability"].get("state"), "HEALTHY")
                self.assertNotIn("process_alive", model.get("runtime") or {})
                self.assertIn('href="/research"', research)
                self.assertIn(">Исследования<", research)
