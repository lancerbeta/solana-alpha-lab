...
CONTRACT_PATH = ROOT / "docs/contracts/task27_historical_collection_authority_contract_v1.md"
CONFIG_PATH = ROOT / "configs/task27_historical_collection_authority_contract_v1.yaml"
SCHEMA_PATH = ROOT / "catalog/schemas/task27_historical_collection_authority.schema.json"
FIXTURE_PATH = ROOT / "tests/fixtures/task27/historical_collection_authority_v1.json"
RECEIPT_PATH = ROOT / "docs/evidence/task27/a0a3_historical_collection_authority_acceptance_v1.json"
...
def semantic_errors(packet: dict[str, Any]) -> set[str]:
    ...
    if packet["decision"]["claim_scope"] != "HISTORICAL_FEASIBILITY_ONLY":
        errors.add("FORBIDDEN_CLAIM_SCOPE")
    return errors


class Task27HistoricalCollectionAuthorityContractTests(unittest.TestCase):
    ...
    def test_descriptive_feasibility_packet_is_admissible_without_pit_claim(self) -> None:
        fixture = json.loads(FIXTURE_PATH.read_text(encoding="utf-8"))
    ...
    def test_config_freezes_authority_evidence_grades_and_caps(self) -> None:
        config = yaml.safe_load(CONFIG_PATH.read_text(encoding="utf-8"))
        self.assertEqual(config["managed_write_set"], EXPECTED_WRITE_SET)
        self.assertEqual(config["evidence_grades"], ["DESCRIPTIVE_ONLY", "PIT_ADMISSIBLE"])
    ...
    def test_receipt_binds_artifacts_and_zero_external_actions(self) -> None:
        ...
        self.assertEqual(receipt["validation"]["adversarial_cases_rejected"], 8)
        self.assertEqual(receipt["validation"]["full_validation"], "PENDING_TRACKED_ONLY_DELIVERY_PREFLIGHT")
        for key in (
            "provider_api_rpc_wss_calls",
            "r2_value_reads",
            "r3_value_or_path_reads",
